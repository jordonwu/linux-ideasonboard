...
static int rkisp1_csi_link_sensor(struct rkisp1_csi *csi, struct v4l2_subdev *sd)
{
	...
	ret = media_entity_get_fwnode_pad(&sd->entity, sd->fwnode,
					  MEDIA_PAD_FL_SOURCE);
	if (ret < 0) {
		dev_err(csi->rkisp1->dev, "failed to find src pad for %s\n",
			sd->name);
		return ret;
	}
	source_pad = ret;

	/* Q: Does this need to be undone at unbind time? */
...
static int
rkisp1_csi_subdev_notifier_bound(struct v4l2_async_notifier *notifier,
				 struct v4l2_subdev *sd,
				 struct v4l2_async_subdev *asd)
{
	struct rkisp1_csi *csi =
		container_of(notifier, struct rkisp1_csi, notifier);
	struct rkisp1_device *rkisp1 = csi->rkisp1;
	...
	ret = rkisp1_csi_link_sensor(csi, s_asd->sd);
	if (ret)
		return ret;
...
static void
rkisp1_csi_subdev_notifier_unbind(struct v4l2_async_notifier *notifier,
				  struct v4l2_subdev *sd,
				  struct v4l2_async_subdev *asd)
{
	struct rkisp1_sensor_async *s_asd =
...
rkisp1_csi_subdev_notifier_ops = {
	.bound = rkisp1_csi_subdev_notifier_bound,
	.unbind = rkisp1_csi_subdev_notifier_unbind,
};

static int rkisp1_csi_subdev_notifier(struct rkisp1_csi *csi)
{
	struct v4l2_async_notifier *ntf = &csi->notifier;
	...
	while (1) {
		...
		ep = fwnode_graph_get_endpoint_by_id(dev_fwnode(csi->rkisp1->dev),
						     0, next_id,
		...
		dev_dbg(csi->rkisp1->dev, "registered ep id %d with %d lanes\n",
...
err_parse:
	...
	if (next_id == 0)
		dev_dbg(csi->rkisp1->dev, "no remote subdevice found\n");
	ntf->ops = &rkisp1_csi_subdev_notifier_ops;
	ret = v4l2_async_nf_register(&csi->rkisp1->v4l2_dev, ntf);
	if (ret) {
		v4l2_async_nf_cleanup(ntf);
		return ret;
	}
...
int rkisp1_csi_register(struct rkisp1_device *rkisp1)
{
	...
	csi = devm_kzalloc(rkisp1->dev, sizeof(*csi), GFP_KERNEL);
	if (!csi)
		return -ENOMEM;
	...
	ret = rkisp1_csi_subdev_notifier(csi);
	if (ret) {
		dev_err(sd->dev,
...
{
	struct v4l2_subdev *sd = rkisp1->csi_subdev;
	struct rkisp1_csi *csi = container_of(sd, struct rkisp1_csi, sd);
